...
sudo dnf install -y utils-linux-user lolcat alacritty fastfetch flatpak ark gnome-tweaks gnome-extensions-app fish zsh
# install multimedia codecs, done separately because some people may have issues with it
sudo dnf install -y gstreamer1-plugins-{bad-\*,good-\*,base} gstreamer1-plugin-openh264 gstreamer1-libav --exclude=gstreamer1-plugins-bad-free-devel

# Display message about installing NVIDIA drivers with lolcat to draw user attention, wait 5s for user to read it, then install NVIDIA drivers
echo "Now comes the NVIDIA driver installation. You'll want to wait a couple minutes as the kernel module is built in the background before rebooting, and hopefully once that's done, you can reboot and have NVIDIA drivers installed." | lolcat
sleep 5s
sudo dnf install akmod-nvidia xorg-x11-drv-nvidia-cuda
echo "NVIDIA drivers should be installed now, and kernel compilation should happen in the background."
